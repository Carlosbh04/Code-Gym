...
export function DashboardHeader({
  ...
  return (
    <header className="relative overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6 lg:p-7">
      <div
        className="pointer-events-none absolute -left-20 top-1/2 size-56 -translate-y-1/2 rounded-full bg-primary/5 blur-3xl"
        aria-hidden="true"
      />
      ...
      <div className="relative grid items-stretch gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.8fr)]">
        ...
        {isLoading ? (
          <div className="grid min-h-28 min-w-0 grid-cols-[5rem_minmax(0,1fr)_2.5rem] items-center gap-3 rounded-xl border border-border bg-background/35 p-3">
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate font-semibold text-foreground">
                {latestActivity.sessionTitle}
              </p>

              <time
                dateTime={latestActivity.completedSession.completedAt}
                className="mt-1 block text-xs text-muted-foreground"
              >
                {formatDate(
                  latestActivity.completedSession.completedAt,
                )}
              </time>
            </div>

            <span
              className="flex size-10 items-center justify-center rounded-lg text-primary transition-colors group-hover:bg-primary/10"
              aria-hidden="true"
            >
